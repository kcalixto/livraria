...
- **Fluxo de status por unidade**: `waiting-payment → in-reserve → payment-received → sent-to-delivery → received`. Unidade `picked_up`: só `waiting-payment → payment-received` (pagamento finaliza direto). **Finalizada** (aba Vendas) = `received` OU (`picked_up` + `payment-received`).
